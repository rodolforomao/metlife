...
class SaldoemprestimosController extends Controller
{
  ...
  if($request->id > 0) { $saldoemprestimo = Saldoemprestimo::findOrFail($request->id); }
  else {
    ...
    $saldoemprestimo->id = $request->id?:0;
      ...
      $saldoemprestimo->created_at = $request->created_at;
  
  
      $saldoemprestimo->updated_at = $request->updated_at;
  
  
      $saldoemprestimo->idcliente = $request->idcliente;
  
  
      $saldoemprestimo->descoberto = $request->descoberto;
  
  
      $saldoemprestimo->maiorperiodo = $request->maiorperiodo;
  
    //$saldoemprestimo->user_id = $request->user()->id;
  $saldoemprestimo->save();

  return redirect('/saldoemprestimos');
